...
| SDK (Python) | Transactions (preview) | 🟢 | n/a | 🟢 | **85%** | round 60 新增 `client.transactions` 命名空间：`append_edits` / `get` / `abort` 三方法包装 OntologyTransaction preview 端点，dataclass `Transaction` + `TransactionAppendResponse`，`?preview=true` 自动附着，`abort` 幂等；剩余 commit 与跨 SDK 异步 client 配对待 server 端 commit 落地 |
