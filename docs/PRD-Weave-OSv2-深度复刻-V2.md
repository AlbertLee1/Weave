...
| 上层体验 | Dashboards / notifications / reactions / permission requests | 🟢 | 🟢 PG | 🟡 | **72%** | `pkg/dashboards` 提供 `/api/v2/dashboards` 全 CRUD + round 62 `POST /api/v2/dashboards/{id}/duplicate` (Foundry "Duplicate" 菜单契约：源可见性按 owner-or-public，clone 始终归当前调用者且 IsPublic 重置，名字自动添加 "(copy)"/"(copy 2)"/"(copy 3)" 后缀避免 409)；`pkg/notifications` + OMS `/api/v2/notifications` 支持通知中心与 fan-out；`pkg/reactions` 提供 `/api/v2/reactions` 给 ObjectDetail ReactionBar；`pkg/permissionrequests` 提供 `/api/v2/permission-requests` request-access workflow；remaining depth gaps 是审计串联、批量治理体验与通知渠道生产化 |
