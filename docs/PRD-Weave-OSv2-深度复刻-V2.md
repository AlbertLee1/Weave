...
| CLI | action / aggregate / objectset | 🟢 | n/a | 🟢 | **85%** | `cmd/weave-cli/cmd_action.go` 暴露 `weave action apply`；`cmd_aggregate.go` 暴露 `weave aggregate`；`cmd_objectset.go` 暴露 `weave objectset load` / `weave objectset create-temporary`；`cmd/weave-cli/cli_us304_test.go` 覆盖命令契约；`docs/cli.md` L141 / L177 / L220 三个章节配命令参考 + body 模板 + 真 northwind 示例（131 行 docs），`scripts/ci/cli_docs_bdd_test.go` 守哨章节存在；Gap-D3 done。剩余 `objectset run` 便捷别名与表格输出 polish |
