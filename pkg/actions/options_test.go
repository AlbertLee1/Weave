...
func TestHandler_Apply_ReturnEdits_NONE(t *testing.T) {
	...
	// Response edits must be empty/nil.
	var resp ApplyResult
	if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
	if len(resp.Edits) != 0 {
		t.Fatalf("returnEdits=NONE must omit edits, got %d", len(resp.Edits))
	}
}
...
func TestHandler_ApplyBatch_Options_ReturnEdits_ALL(t *testing.T) {
	...
	if len(br.AppliedEdits) == 0 {
		t.Fatal("returnEdits=ALL must include appliedEdits")
	}
}
...
func TestHandler_ApplyBatch_Options_ReturnEdits_NONE(t *testing.T) {
	...
	if len(br.AppliedEdits) != 0 {
		t.Fatalf("returnEdits=NONE must omit appliedEdits, got %d", len(br.AppliedEdits))
	}
	for i, r := range br.Results {
		if len(r.Edits) != 0 {
			t.Fatalf("results[%d] must have no edits with returnEdits=NONE, got %d", i, len(r.Edits))
		}
	}
}
...
func TestHandler_ApplyBatch_DefaultOptions_AtomicWithEdits(t *testing.T) {
	...
	if len(br.AppliedEdits) == 0 {
		t.Fatal("default options must include appliedEdits")
	}
}
