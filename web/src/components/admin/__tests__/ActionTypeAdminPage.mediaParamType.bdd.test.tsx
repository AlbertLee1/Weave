...
// BDD: the ActionType admin builder must let an author declare a `media`-typed
// parameter. The v2 backend persists `parameters` as opaque json.RawMessage,
// and the runtime ParameterForm already renders a media upload control for
// `type: 'media'`, but the builder's PARAMETER_TYPES dropdown previously
// stopped at `attachment` — leaving no way to declare a media parameter.
//
// Given the Ontology Manager ActionType builder
// When  the author adds a parameter and opens its type dropdown
// Then  a `media` option is offered, and selecting it makes the create
//       (and edit) request body carry that parameter with type:'media'.
...
const ACTION_TYPES = [
  {
    rid: 'ri.ontology.main.action-type.uploadAvatar',
    apiName: 'uploadAvatar',
    displayName: 'Upload Avatar',
    status: 'ACTIVE',
    // Existing action whose parameter is already media-typed, so the edit
    // path can prove the dropdown pre-selects and round-trips `media`.
    parameters: {
      avatar: { dataType: { type: 'media' }, required: true },
    },
    rules: [],
  },
];
...
describe('ActionTypeAdminPage media parameter type', () => {
  let state: StubState;
  ...
  it('offers a media option in the parameter type dropdown', async () => {
    ...
    const mediaOption = within(typeSelect as HTMLSelectElement).getByRole(
      'option',
      { name: 'media' },
    );
  ...
  it('create body carries a media-typed parameter when media is selected', async () => {
    const user = userEvent.setup();
    renderPage();
    await waitFor(() => {
      expect(screen.getByText('Upload Avatar')).toBeInTheDocument();
    ...
    const photo = params.find((p) => p.id === 'photo');
    expect(photo).toBeDefined();
    expect(photo?.type).toBe('media');
  });

  it('edit modal pre-selects media and round-trips it on save', async () => {
    const user = userEvent.setup();
    renderPage();
    await waitFor(() => {
      expect(screen.getByText('Upload Avatar')).toBeInTheDocument();
    });
    const row = screen.getByText('Upload Avatar').closest('tr')!;
    await user.click(within(row).getByRole('button', { name: /Edit/i }));

    const typeSelect = (await screen.findByLabelText(
      /Parameter 1 type/i,
    )) as HTMLSelectElement;
    expect(typeSelect.value).toBe('media');
    ...
    const params = body.parameters as Array<{ id: string; type: string }>;
    const avatar = params.find((p) => p.id === 'avatar');
